...
if [ ! -x node_modules/.bin/vite ]; then
  if [ "$mode" = "check" ]; then
    echo "Error: standalone dependencies are not installed. Run ./start-demo.sh once." >&2
    exit 1
  fi
  echo "Installing standalone voice-room dependencies..."
  npm install
fi
...
if [ "$mode" = "check" ]; then
  ...
  echo "Standalone package: demos/voice-room"
...
echo "Starting standalone Agora voice-room demo..."
