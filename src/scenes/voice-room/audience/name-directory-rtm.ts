...
import type { DirectoryTransport } from '../name-directory';
import type { TraceEntry, TraceInput } from '../../../shared/timeline/traceStore';

/** audience directory operations share the page client, independently of room listeners. */
export class AudienceNameDirectoryRtm implements DirectoryTransport {
  ...
  private record(input: TraceInput): void {
    this.traces.push({ ...input, seq: ++this.seq, uid: this.session.userId, role: 'audience' });
    this.notify();
  }
