...
export interface HostRtmEventListeners {
  /** Updates the Host business store's normalized connection state. */
...
export interface HostOnRtmEventOptions {
  roomId: string;
  userId: string;
  source: AppRtmEventSource;
  listeners: HostRtmEventListeners;
...
/** Binds Host SDK events, validates protocol context, and invokes store listeners. */
export class HostOnRtmEvent implements HostRtmEventBinding {
  private readonly now: () => number;
  private readonly acceptedMessages = new Set<string>();

  /** Captures the Host event source, store listeners, and TTL clock. */
  constructor(private readonly options: HostOnRtmEventOptions) {
    this.now = options.now ?? (() => Date.now());
  }

  /** Installs all Host event listeners on the page-level event source. */
  bind(observer: HostRtmEventObserver): () => void {
    return this.options.source.bindRtmEvents(this.eventHandlers(observer));
  }

  /** Creates the complete SDK listener map for the current Host room binding. */
  private eventHandlers(observer: HostRtmEventObserver): AppRtmEventListeners {
    return {
      /** Normalizes link state and forwards it to the Host store. */
      linkState: (event) => {
  ...
  /** Accepts only this Host's USER channel or the bound room MESSAGE channel. */
...
/** Creates the Host event-binding module for one room role instance. */
export function createHostOnRtmEvent(options: HostOnRtmEventOptions): HostOnRtmEvent {
  return new HostOnRtmEvent(options);
}
